...
easy-add --var version=${brc_version} --from https://github.com/Pugmatt/BedrockConnect/releases/download/{{.version}}/${brc_source_zipfile} --file ${brc_source_file} -to /config
chmod 664 /config/BedrockConnect-1.0-SNAPSHOT.jar
...
java -Xms${java_xms} -Xmx${java_xmx} -jar /config/BedrockConnect-1.0-SNAPSHOT.jar \
